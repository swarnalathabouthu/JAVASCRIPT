...
let cartItems = ['KurthiSet', 'Jeans', 'CropTop', 'WestenDress'];
...
//02. Hotstar: Subscription Plan Selection (Switch Case & Prompt)
let plan = prompt("Choose a subscription plan: Basic, Premium, or VIP");
switch (plan) {
    case 'Basic':
        alert("You selected the Basic Plan: $10/month")
        break;
    case 'Premium':
        alert("You selected the Premium Plan: $20/month");
        break;
    case 'VIP':
        alert("You selected the VIP Plan: $5/month");
        break;
    default:
        alert("Invalid selection. Please choose a valid plan.");
}

//Uber (Ride Booking Experience).
...
if (confirmBooking) {
    ...
    setTimeout(confirmRideBooking, 2000);
} else {
    alert("Booking cancelled.");
}

//04 Amazon Product Rating Feature.
let rating = prompt("Please rate the product from 1 to 5.");
for (let i = 1; i <= 5; i++) {
    if (i <= rating) {
...
//04 Hotstar (Video Quality Selection).
